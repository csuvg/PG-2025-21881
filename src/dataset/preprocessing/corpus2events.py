    ...
    for root, _, files in os.walk(root_dir):
        for file in files:
            if file.endswith(extension):
                ...
                pure_path = os.path.relpath(mix_path, root_dir) if is_pure else mix_path
                if not is_ext:
...
def corpus2event_cp(path_infile, path_outfile):
    ...
    # Map emotion tag; fallback to Q1 if unknown
    raw_emo = data['metadata'].get('emotion', 'Q1')
    emo_tag = emo_map.get(raw_emo, 1)
    
    # process
    final_sequence = []
    final_sequence.append(create_emo_event(emo_tag))
    # insert global Key event (if available)
...
if __name__ == '__main__':
    # paths
    import sys
    # Usage: python corpus2events.py <input_dir> <output_dir>
    # Example: python corpus2events.py ./dataset/pre/corpus ./dataset/pre/events
    path_indir = sys.argv[1] if len(sys.argv) > 1 else 'corpus'
    path_outdir = sys.argv[2] if len(sys.argv) > 2 else 'events'
    path_root = os.path.dirname(path_outdir) or '.'
