...
def write_midi(words, path_outfile, word2event):
    ...
    for i in range(len(words)):
        ...
        if vals[3] == 'Metrical':
            if vals[2] == 'Bar':
                bar_cnt += 1
            elif isinstance(vals[2], str) and 'Beat' in vals[2]:
...
def sampling(logit, p=None, t=1.0, is_training=False):
    """
    Sample from logits with optional temperature and nucleus sampling.
    
    Args:
        logit: logits tensor/array
        p: nucleus sampling threshold (None = no nucleus)
        t: temperature (None or 1.0 = no temperature scaling)
        is_training: whether in training mode
    """
    # Handle None temperature by using default
    if t is None:
        t = 1.0
    ...
    if is_training:
        ...
        # For very low temperature (deterministic mode), use argmax directly
